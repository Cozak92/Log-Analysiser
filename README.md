...
로컬에서 실제 Kibana 없이 흐름을 확인하려면 어드민 화면에서 Kibana URL에 `demo://local`, Data view name에 `payments-*` 또는 `db-*`를 입력한 뒤 `Poll now`를 누르면 샘플 로그 기반 탐지 목록이 생성됩니다.
...
```bash
curl -X POST http://127.0.0.1:8000/admin/api/sources ^
  -H "Content-Type: application/json" ^
  -d "{\"kibana_url\":\"demo://local\",\"data_view_name\":\"db-*\",\"analyzer_mode\":\"mock\",\"llm_provider\":\"mock\",\"llm_model\":null}"
```
...
Source-level LLM provider options are available in the admin page. Built-in choices include `mock`, `openai`, `anthropic`, `gemini`, `azure-openai`, `bedrock`, `vertex-ai`, `openrouter`, and `ollama`. Select `custom` to store a custom provider name such as an internal LLM gateway.
...
구성에는 FastAPI 앱과 MongoDB가 포함됩니다. 앱은 등록된 Kibana source를 10초 주기로 polling하고, 분석 결과가 `high` 또는 `critical`이거나 명확한 에러 유형으로 분류되면 MongoDB에 탐지 항목으로 저장합니다.
...
3. 서비스/언어별 규칙 세분화
4. 반복 발생 집계와 incident correlation 추가
5. 승인 기반 code suggestion export 기능 추가
